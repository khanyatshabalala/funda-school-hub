...
const parentItems = [
  ...
  { to: "/app/marks", title: "Marks & reports", icon: BookOpen },
  ...
  { to: "/app/alerts", title: "Alerts", icon: Bell },
...
const schoolBaseItems = [
  ...
  { to: "/app/marks-capture", title: "Marks", icon: BookOpen },
...
const adminExtra = [
  ...
  { to: "/app/staff", title: "Staff", icon: Users },
      ...
      <SidebarFooter className="border-t border-sidebar-border p-3">
        <div className="flex items-center justify-between text-xs">
          <div className="truncate">
            <div className="font-medium text-sidebar-foreground truncate">{profile?.full_name ?? "User"}</div>
            <div className="text-sidebar-foreground/60 capitalize">{primaryRole.replace("_"," ")}</div>
          </div>
          <Button size="icon" variant="ghost" onClick={() => signOut()} className="text-sidebar-foreground hover:bg-sidebar-accent"><LogOut className="size-4"/></Button>
        </div>
      </SidebarFooter>
    </Sidebar>
  );
