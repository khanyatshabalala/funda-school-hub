...
function SchoolAuth() {
  ...
  const onSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Route super_admin to the admin console, everyone else to the app
      ...
      <div className="hidden lg:flex bg-primary text-primary-foreground p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h2 className="text-3xl font-bold leading-tight">
            For teachers, principals & school admins.
          </h2>
      ...
      <div className="flex items-center justify-center p-6 bg-background">
        <Card className="p-8 w-full max-w-md">
          ...
          <form onSubmit={onSignIn} className="space-y-4 mt-6">
            <div>
              <Label htmlFor="se">Work email</Label>
              <Input id="se" name="email" type="email" required autoComplete="email" />
            </div>
            <div>
              <Label htmlFor="sp">Password</Label>
              <Input id="sp" name="password" type="password" required autoComplete="current-password" />
            </div>
